module sync_fifo (
    ...
    input wire rd_en, output wire [7:0] rd_data,
    ...
    assign rd_data = mem[head];          // BUG: combinational, not registered
    wire wr_ok = wr_en && !full;
    wire rd_ok = rd_en && !empty;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin head <= 3'd0; tail <= 3'd0; count <= 4'd0;
        end else begin
            if (rd_ok) head <= head + 3'd1;
